 ...
 * @return img
 */
static struct image_t *object_detector(struct image_t *img, unsigned char p)
{
  bool draw = cod_draw1;
...
void cyberzoo_carpet_init(void)
{
  ...
  cv_add_to_device(&CYBERZOO_CARPET_CAMERA1, object_detector, CYBERZOO_CARPET_FPS1, 0);
...
float find_carpet(struct image_t *img, bool draw)
{
  ...
  int16_t total = 0;
  for (i=26-12;i<26+12;i++) {
    total += confidence[i];
  }
  carpet_land_colliding = ((float)total) / 24.0f;
